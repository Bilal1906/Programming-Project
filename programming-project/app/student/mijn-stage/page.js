        ...
        <div className="bg-white rounded-xl p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-semibold text-blue-600">
              {Math.round((new Date() - new Date(stage.startdatum)) / (new Date(stage.einddatum) - new Date(stage.startdatum)) * 100)}% 
            </span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
            <div
              className="bg-[#1e3a5f] h-2 rounded-full"
              style={{ width: `${Math.min(100, Math.round((new Date() - new Date(stage.startdatum)) / (new Date(stage.einddatum) - new Date(stage.startdatum)) * 100))}%` }}
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl p-5">
            ...
            <div className="space-y-4">
              {[
                {
                  initialen: `${stage.mentor_voornaam?.[0]}${stage.mentor_achternaam?.[0]}`,
                  naam: `${stage.mentor_voornaam} ${stage.mentor_achternaam}`,
                  functie: `Stagementor · ${stage.bedrijf_naam}`,
                  email: stage.mentor_email,
                },
                {
                  initialen: `${stage.docent_voornaam?.[0]}${stage.docent_achternaam?.[0]}`,
                  naam: `${stage.docent_voornaam} ${stage.docent_achternaam}`,
                  functie: 'Begeleider EhB · Docent',
                  email: stage.docent_email,
                },
              ].map((persoon) => (
