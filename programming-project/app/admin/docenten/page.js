...
export default function DocentenPage() {
  ...
  const handleVerwijder = async () => {
    ...
    if (window.confirm(`Weet u zeker dat u ${geselecteerd.length} docent(en) wilt verwijderen?`)) {
      ...
      setDocenten(prev => prev.filter(d => !geselecteerd.includes(d.id)));
      setGeselecteerd([]);
    ...
    <main className="flex-1 flex flex-col">
      ...
      <div className="flex-1 p-6 bg-gray-50">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-1">Docentenoverzicht</h2>
            <p className="text-sm text-gray-400">Voeg toe, bewerk en verwijder één of meerdere docent(en)</p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left px-5 py-3 text-xs font-medium text-gray-400">Docent</th>
