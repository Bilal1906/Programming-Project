...
export default function StudentBewerkenPage() {
  ...
  const [form, setForm] = useState({
    ...
    opleiding: '', academiejaar: '', adres: '',
  });

  useEffect(() => {
    if (!id) return;
    fetchMetAuth(`/api/admin/studenten/${id}`)
      .then(res => res?.json())
      .then(data => {
        if (data && !data.fout) {
          setForm({
  ...
  const handleOpslaan = async () => {
    ...
    const response = await fetchMetAuth(`/api/admin/studenten/${id}`, {
      method: 'PUT',
      body: JSON.stringify({
  ...
  return (
    <main className="flex-1 flex flex-col">
      ...
      <div className="flex-1 p-6 bg-gray-50 overflow-y-auto">
        <h2 className="text-2xl font-bold text-gray-900 mb-1">Student bewerken</h2>
        <p className="text-sm text-gray-400 mb-6">Bewerk de gegevens van een student</p>
