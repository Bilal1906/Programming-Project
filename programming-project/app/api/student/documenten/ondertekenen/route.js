...
export async function PUT(request) {
  try {
    ...
    if (bestaand.length === 0) {
      await db.query(
        `INSERT INTO overeenkomst (stage_id, signed_student, created_at) VALUES (?, 1, NOW())`,
        [stage_id]
      )
    } else {
      await db.query(
        `UPDATE overeenkomst SET signed_student = 1, signed_at = IF(signed_stagementor = 1, NOW(), signed_at) WHERE stage_id = ?`,
        [stage_id]
      )
    }
