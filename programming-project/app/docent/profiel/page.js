...
export default function DocentProfiel() {
  ...
  return (
    <div className="flex-1 flex flex-col">
      <DocentTopbar titel="Mijn Profiel" subtitel="2025-2026 · Erasmushogeschool Brussel" />
        ...
        <div className="max-w-2xl space-y-4">
          ...
          <div className="bg-white rounded-xl p-5">
            ...
            <form onSubmit={handleWachtwoordOpslaan} className="space-y-3">
              <div><label className="block text-xs text-gray-400 mb-1">Huidig wachtwoord</label><input type="password" placeholder="Uw huidig wachtwoord" value={huidigWachtwoord} onChange={(e) => setHuidigWachtwoord(e.target.value)} className={inputStyle} /></div>
              <div><label className="block text-xs text-gray-400 mb-1">Nieuw wachtwoord</label><input type="password" placeholder="Kies een nieuw wachtwoord" value={nieuwWachtwoord} onChange={(e) => setNieuwWachtwoord(e.target.value)} className={inputStyle} /></div>
              <div><label className="block text-xs text-gray-400 mb-1">Bevestig nieuw wachtwoord</label><input type="password" placeholder="Herhaal uw nieuw wachtwoord" value={bevestigWachtwoord} onChange={(e) => setBevestigWachtwoord(e.target.value)} className={inputStyle} /></div>
              {fout && <div className="text-red-500 text-xs">{fout}</div>}
              {bericht && <div className="text-green-500 text-xs">{bericht}</div>}
              <button type="submit" className="px-5 py-2 bg-[#1e3a5f] text-white text-sm rounded-lg hover:bg-[#162d4a] cursor-pointer font-medium">Wachtwoord opslaan</button>
            </form>
          </div>

          <button onClick={handleUitloggen} className="px-4 py-2 bg-white border border-gray-200 text-red-500 text-sm rounded-lg hover:bg-red-50 cursor-pointer font-medium flex items-center gap-2">
            ⏻ Uitloggen
          </button>
        </div>
      </div>
