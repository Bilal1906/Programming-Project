...
export default function DocentEvaluatieDetail() {
  ...
  const [presentatieScore, setPresentatieScore] = useState('')
    ...
    fetchMetAuth(`/api/docent/evaluaties/${id}`)
      ...
      .then(data => {
        if (data && !data.fout) {
          ...
          setPresentatieScore(data.evaluatie.presentatie_score ?? '')
        }
        setLoading(false)
      })
  ...
  const handleOpslaan = async () => {
    ...
    const response = await fetchMetAuth('/api/docent/evaluaties', {
      ...
      body: JSON.stringify({
        ...
        presentatie_datum: evaluatie.type === 'finaal' ? presentatieDatum || null : undefined,
        presentatie_notities: evaluatie.type === 'finaal' ? presentatieNotities || null : undefined,
        presentatie_score: evaluatie.type === 'finaal' && presentatieScore !== '' ? parseFloat(presentatieScore) : undefined,
      }),
    })
  ...
  const isVerlopen = evaluatie?.datum && new Date() > new Date(new Date(evaluatie.datum).toDateString() + ' 23:59:59')
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 bg-gray-100 p-6 space-y-4 overflow-y-auto">
            ...
            <div key={s.competentie_id} className="bg-white rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-600">{s.score_mentor ?? '—'}</span>
                </div>
              </div>

              {s.rubriek.length > 0 ? (
                <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${s.rubriek.length}, 1fr)` }}>
                  {s.rubriek.map(niveau => {
                    const geselecteerd = parseFloat(s.score_docent) === parseFloat(niveau.score)
                    return (
                      <button
                        key={niveau.id}
                        type="button"
                ...
                <div className="flex items-center gap-3">
                  <p className="text-xs text-gray-400">Geen rubriek beschikbaar — geef een score:</p>
                  <input
              ...
              {s.score_docent !== null && s.score_docent !== '' && (
                <div className="mt-2 text-xs text-gray-500">
                  Geselecteerde score: <span className="font-semibold text-[#1e3a5f]">{s.score_docent}</span>
                </div>
              )}
            </div>
          ))}
        ...
        {/* Eindpresentatie — alleen bij finaal */}
        {evaluatie.type === 'finaal' && (
          <div className="bg-white rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-800 mb-1">Eindpresentatie</h2>
            <p className="text-xs text-gray-400 mb-4">Registreer de eindpresentatie van de student.</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-gray-400 mb-1">Datum presentatie</label>
                <input
                  type="date"
                  disabled={isVerlopen}
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Score presentatie (0-20)</label>
                <input
                  type="number" min="0" max="20" step="0.5"
                  disabled={isVerlopen}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400 disabled:bg-gray-50 text-center"
                  placeholder="0-20"
                  value={presentatieScore}
                  onChange={e => setPresentatieScore(e.target.value)}
                />
              </div>
              <div className="col-span-2">
                <label className="block text-xs text-gray-400 mb-1">Notities / feedback presentatie</label>
                <textarea
                  disabled={isVerlopen}
                  ...
                  placeholder="Notities over de eindpresentatie..."
