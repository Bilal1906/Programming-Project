...
const statusBadge = (status) => {
  const stijlen = { voltooid: 'bg-green-50 text-green-700 border border-green-200', open: 'bg-yellow-50 text-yellow-700 border border-yellow-200' }
  return <span className={`text-xs px-2 py-1 rounded-full font-medium ${stijlen[status] || stijlen.open}`}>{status === 'voltooid' ? 'Voltooid' : 'Open'}</span>
}
      ...
      <div className="flex-1 bg-gray-100 p-6 space-y-4">
        <div className="bg-white rounded-xl p-5">
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={() => router.push('/docent/evaluaties/nieuw')} className="px-4 py-2 bg-[#1e3a5f] text-white text-sm rounded-lg cursor-pointer font-medium">Nieuwe Evaluatie</button>
          </div>
        </div>
        <div className="bg-white rounded-xl p-5">
          {evaluaties.length === 0 ? (
              ...
              <tbody>
                {evaluaties.map((e) => (
                  <tr key={e.id} className="border-b border-gray-50">
                    ...
                    <td className="py-3"><button className="px-3 py-1.5 bg-[#1e3a5f] text-white text-xs rounded-lg cursor-pointer">{e.status === 'open' ? 'Invullen' : 'Bekijken'}</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
